 ...
 This project requires very little clean up to use it as a start point for
 a new project. The only files you will need to consider changing is the 
 home.xhtml file and the template file :
 
 WEB-INF\templates\template.xhtml
 
 and the stylesheet in :
 
 webapp\resources\css\screen.css
 ...
 provides a simple smoke test for JSF, CDI and JPA.

 The included code is a CDI qualifier for the data repository producer that 
 produces an entity manager as per the CDI specification.
 ...
 http://localhost:8080/mateo/home.jsf
 ...
 2) Manually - If you run the Maven command :
    ...
    manually deploy the resulting target/mateo.war file to your server
